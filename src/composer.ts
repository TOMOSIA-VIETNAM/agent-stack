...
export interface Manifest {
  ...
  extras: string[];
  imported: { source: string; repo: string; ref: string; license: string; licenseUrl: string }[];
}
...
/**
 * Attribution for a copied file, carried on the file itself. The upstream
 * licences require the copyright notice to travel with every copy, so this line
 * is not decoration: it is the condition under which the content may be
 * redistributed at all.
 */
function originComment(entry: SelectedArtifact): string {
  const { provenance, source } = entry.artifact;
  return provenance
    ? `<!-- ${provenance.copyright}. ${provenance.license} licensed: ${provenance.licenseUrl}. Copied from ${provenance.repo} at ${provenance.ref} -->`
