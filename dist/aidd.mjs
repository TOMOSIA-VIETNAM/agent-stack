...
function originComment(entry) {
  ...
  return provenance ? `<!-- ${provenance.copyright}. ${provenance.license} licensed: ${provenance.licenseUrl}. Copied from ${provenance.repo} at ${provenance.ref} -->` : `<!-- generated by open-aidd from knowledge/${source} - edit the knowledge base, not this file -->`;
}
...
function compose(stack, selection, generatorVersion, imported = []) {
  ...
  const manifest = {
    ...
    extras: [],
    imported: used.map(({ name, repo, ref, license, licenseUrl }) => ({
      source: name,
      repo,
      ref,
      license,
  ...
  for (const id of ids) {
    ...
    for (const other of tech.conflicts_with) {
      ...
      conflicts.push({
        ...
        left,
        right,
        message: `${catalog.technologies[left]?.name ?? left} and ${catalog.technologies[right]?.name ?? right} are declared incompatible`
      });
    }
  }
...
function conflictFinding(conflict, isAccepted) {
  return {
...
var USAGE = `open-aidd ${VERSION} - generate .claude rules and skills from a curated knowledge base
